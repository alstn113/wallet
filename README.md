...
분산 환경에서 발생할 수 있는 문제점들을 해결하고, 결과적 정합성을 보장하는 안전한 이체 시스템을 설계 및 구현합니다.
...
1. transfer-app (이체 요청)
2. transfer-app -> wallet-app (출금 요청 / HTTP)
    - 요청은 동기 방식, 실패 시 메시징 방식으로 보상 트랜잭션 처리
3. transfer-app -> wallet-app (입금 요청 / HTTP)
    - 요청은 동기 방식, 실패 시 메시징 방식으로 보상 트랜잭션 처리
4. transfer-app (이체 결과 응답)
